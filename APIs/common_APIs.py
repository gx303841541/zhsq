...
def modify_cls(casename, log_dir):
    def cls_decorator(cls):
        def __init__(self):
            super(cls, self).__init__(
                name=casename, log_dir=log_dir)
...
def modify_init(casename, log_dir):
    def cls_decorator(cls):
        def _init_(self):
            super(cls, self)._init_(
                name=casename, log_dir=log_dir)
...
def dirit(dir):
    if not dir.endswith(os.path.sep):
        dir += os.path.sep
    # return dir
    return re.sub(r'%s+' % (re.escape(os.path.sep)), re.escape(os.path.sep) + re.escape(os.path.sep), dir, re.S)
